...
processedData = open('../processedOriData/processedOriData.pkl', 'rb')
...
with tf.Session(config=session_conf) as session:
    ...
    for epoch in range(training_epochs):
        for b in range(total_batches):    
            ...
            summary, acc = session.run([merged, accuracy], feed_dict={X: train_x[train_indices], Y: train_y[train_indices]})
